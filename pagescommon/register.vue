	...
	export default {
		...
		methods: {
			...
			getVerCode(){
				...
				_this.isRotate=true
				b.getHttpWithLocation("db=本地圈儿1004&action=sms&phone="+this.phoneData,
					(data,error)=>{ 
		    ...
		    startReg() {
				...
				b.getHttpWithLocation("db=本地圈儿1004&action=register&phone="+this.phoneData+"&password="+this.passData+"&name="+this.nickname,
					(data,error)=>{ 
					c.show(data.Status)
					if(data.Status==1)
					{
